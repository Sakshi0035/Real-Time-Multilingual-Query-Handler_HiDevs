...
const TranslationCard: React.FC<TranslationCardProps> = ({ query }) => {
  ...
  return (
    <div className="group relative bg-white rounded-2xl border border-zinc-200 overflow-hidden transition-all duration-300 hover:shadow-lg">
      ...
      <div className="p-6">
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center space-x-3">
            <div className="px-2 py-1 bg-zinc-50 border border-zinc-100 rounded-md">
              <span className="text-[10px] font-black uppercase tracking-tighter text-zinc-700">
        ...
        <div className="space-y-6">
          <div className="relative pl-4 border-l-2 border-zinc-100">
            <h4 className="text-[10px] font-bold text-zinc-600 uppercase tracking-widest mb-2">Captured Signal</h4>
            <p className="text-zinc-700 italic text-sm leading-relaxed">"{query.originalText}"</p>
          </div>
          ...
          {query.status === 'pending' ? (
            ...
            <div className="space-y-6">
              <div className="bg-zinc-50 p-5 rounded-2xl border border-zinc-100">
                <h4 className="text-[10px] font-bold text-violet-600 uppercase tracking-widest mb-2">Refined English Output</h4>
                <p className="text-zinc-900 font-medium text-base leading-relaxed tracking-tight">{query.translatedText}</p>
              </div>
              ...
              {query.suggestedResponse ? (
                <div className="pt-2">
                  ...
                  <div className="bg-zinc-50 p-5 rounded-2xl border border-fuchsia-50 transition-all duration-300">
                    <p className="text-zinc-700 text-sm leading-relaxed">{query.suggestedResponse}</p>
                    <button 
                      onClick={() => navigator.clipboard.writeText(query.suggestedResponse)}
                      className="mt-4 px-3 py-1.5 rounded-lg text-[10px] font-bold bg-white text-zinc-700 border border-zinc-100 hover:bg-zinc-50 transition-all flex items-center uppercase tracking-widest"
                    >
